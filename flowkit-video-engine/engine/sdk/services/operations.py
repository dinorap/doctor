...
from engine.db import crud
from engine.config import VIDEO_POLL_INTERVAL, VIDEO_POLL_TIMEOUT
from engine.utils.paths import scene_4k_path
from engine.utils.slugify import slugify
from engine.worker._parsing import (
    _is_error,
    _is_uuid,
...
if TYPE_CHECKING:
    from engine.services.flow_client import FlowClient
...
def _extract_operations(result: dict) -> list[dict]:
    ...
    - Old: data.operations[] with operation.name
    - New v3.1+: data.media[] with video.operation.name
    """
    data = result.get("data", result)

    # Try new v3.1+ format first: media[]
    ...
    if isinstance(media, list) and media:
        ...
        for item in media:
            if not isinstance(item, dict):
                continue
            # Extract operation name from video.operation.name
            video = item.get("video", {})
            operation = video.get("operation", {}) if isinstance(video, dict) else {}
            op_name = operation.get("name")
            # Status is nested: mediaMetadata.mediaStatus.mediaGenerationStatus
            media_metadata = item.get("mediaMetadata", {})
            media_status = media_metadata.get("mediaStatus", {})
            status = media_status.get("mediaGenerationStatus", item.get("status", ""))
            # Get video/thumbnail URLs if available
            video_url, thumbnail_url = _extract_media_from_status_op(item)
            ops.append({
                "operation": {"name": op_name or ""},
                "status": status,
                "videoUrl": video_url,
                "thumbnailUrl": thumbnail_url,
            })
            if not op_name:
                logger.warning("Media item missing operation.name: %s", item)
        return ops

    # Fallback to old format: operations[]
...
def _extract_media_from_status_op(op: dict) -> tuple[str, str]:
    """Extract video/thumbnail URL from a status operation item.

    Supports both formats:
    - New (media item): op has "video" key directly
    - Old (operation item): op has "operation" -> "metadata" -> "video"
    """
    video_url = ""
    thumbnail_url = ""

    # NEW FORMAT: media item with "video" key directly
    if "video" in op and isinstance(op.get("video"), dict):
        video = op["video"]

        # Video URL: generatedVideo.fifeUrl or video.fifeUrl
        generated_video = video.get("generatedVideo", {})
        if isinstance(generated_video, dict) and generated_video.get("fifeUrl"):
            video_url = str(generated_video["fifeUrl"])
        if not video_url and video.get("fifeUrl"):
            video_url = str(video["fifeUrl"])

        # Thumbnail URL: generatedImage.fifeUrl or poster.fifeUrl or servingBaseUri
        generated_image = video.get("generatedImage", {})
        if isinstance(generated_image, dict) and generated_image.get("fifeUrl"):
            thumbnail_url = str(generated_image["fifeUrl"])
        if not thumbnail_url:
            poster = video.get("poster", {})
            if isinstance(poster, dict) and poster.get("fifeUrl"):
    ...
    # OLD FORMAT: operation item with "operation" -> "metadata" -> "video"
            ...
            if isinstance(video, dict):
                ...
                if not thumbnail_url:
                    ...
                    if isinstance(poster, dict) and poster.get("fifeUrl"):
                        thumbnail_url = str(poster["fifeUrl"])
                if not thumbnail_url:
                    serving = str(video.get("servingBaseUri") or "")
                    if serving:
                        thumbnail_url = serving
                    else:
...
async def _poll_operations(
    ...
    - Old: response.data.operations[]
    - New v3.1+: response.data.media[] with status and video URLs
    """
    if not operations:
        ...
        if isinstance(media, list) and media:
            ...
            for item in media:
                ...
                if status == "MEDIA_GENERATION_STATUS_SUCCESSFUL":
                    vu, tu = _extract_media_from_status_op(item)
                    if vu:
                        video_url = vu
                    if tu:
                        thumbnail_url = tu
                    ...
                    logger.error("Media item FAILED: name=%s full=%s", op_name, _json.dumps(item)[:1000])
                    error_msg = f"Media failed: {op_name}"
                    has_error = True
                    break
                else:
                    all_done = False
            ...
            if all_done:
                logger.info("All %d media items completed after %ds", len(media), elapsed)
                # Return with URLs if found
                result_data = dict(data)
                if video_url:
                    result_data["videoUrl"] = video_url
                if thumbnail_url:
                    result_data["thumbnailUrl"] = thumbnail_url
                return {"data": result_data}

            done_count = sum(1 for m in media if m.get("mediaMetadata", {}).get("mediaStatus", {}).get("mediaGenerationStatus") == "MEDIA_GENERATION_STATUS_SUCCESSFUL")
            logger.debug("Poll %ds/%ds (media): %d/%d done", elapsed, timeout, done_count, len(media))
            # Update current_ops to match the media items for next poll
            current_ops = [{"operation": {"name": item.get("name") or item.get("video", {}).get("operation", {}).get("name") or ""}, "status": item.get("mediaMetadata", {}).get("mediaStatus", {}).get("mediaGenerationStatus", item.get("status", ""))} for item in media]
            continue
