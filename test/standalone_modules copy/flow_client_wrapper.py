"""
Flow Client Wrapper - Standalone Module
Simplified wrapper cho Google Flow API qua WebSocket.
"""
import asyncio
import logging
...
class FlowClientWrapper:
    """Wrapper client cho Google Flow API qua WebSocket."""
    ...
    async def _send(self, method: str, params: dict, timeout: float = None) -> dict:
        """Send request qua WebSocket và đợi response."""
    ...
    def _client_context(self, project_id: str, tier: str = "PAYGATE_TIER_TWO") -> dict:
        """Build client context cho API requests."""
        return {
    ...
    async def create_project(self, title: str, tool_name: str = "PINHOLE") -> dict:
        ...
        return await self._send("createProject", params)
    ...
    async def generate_video(
        ...
        params = {
            **self._client_context(project_id, user_paygate_tier),
            "startImageMediaId": start_image_media_id,
            "prompt": prompt,
            "sceneId": scene_id,
            "aspectRatio": aspect_ratio,
        }
        if end_image_media_id:
            params["endImageMediaId"] = end_image_media_id
        
        return await self._send("generateVideo", params)
    ...
    async def check_video_status(self, operations: list[dict]) -> dict:
        ...
        return await self._send("checkVideoStatus", params)
