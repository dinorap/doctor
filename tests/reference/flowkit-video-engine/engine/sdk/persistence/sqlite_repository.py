"""SQLiteRepository — wraps engine.db.crud for SDK persistence."""
...
class SQLiteRepository(Repository):
    """Concrete repository backed by the SQLite DB via engine.db.crud."""
    ...
    async def list(self, table: str, **filters: Any) -> list[dict[str, Any]]:
        ...
        from engine.db.schema import get_db
        db = await get_db()
    ...
    async def insert(self, table: str, row: dict[str, Any]) -> dict[str, Any]:
        from engine.db.schema import get_db, _db_lock
        db = await get_db()
        cols = list(row.keys())
