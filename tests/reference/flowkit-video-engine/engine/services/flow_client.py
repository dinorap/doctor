...
from engine.services.headers import random_headers
...
class FlowClient:
    ...
    async def refresh_project_urls(self, project_id: str) -> dict:
        ...
        """
        from engine.db import crud
        from engine.utils.media_recovery import check_and_recover_media, get_local_path, is_url_expired
    ...
    async def check_video_status(self, operations: list[dict]) -> dict:
        ...
        Sends the operations in the format they came in:
        - Old format: {"operations": operations}
        - New v3.1+ format: {"media": operations} with operation.name as mediaId

        The response will have either operations[] or media[] which the caller
        (operations.py _poll_operations) knows how to parse.
        """
        # Detect if these are new-format media items (have "name" or "video" key)
        # vs old-format operation items (have "operation.name")
        has_old_format = any(
            isinstance(op, dict) and isinstance(op.get("operation"), dict) and op["operation"].get("name")
            for op in operations
        )
        has_new_format = any(
            isinstance(op, dict) and (op.get("name") or op.get("video"))
            for op in operations
        )

        if has_new_format and not has_old_format:
            # New v3.1+ format: send as media array
            body = {"media": operations}
        else:
            # Old format: send as operations array
