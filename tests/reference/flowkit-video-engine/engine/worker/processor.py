...
from engine.config import POLL_INTERVAL, MAX_RETRIES, API_COOLDOWN, MAX_CONCURRENT_REQUESTS
from engine.worker._parsing import _is_error
from engine.sdk.services.result_handler import parse_result, apply_scene_result, apply_character_result
...
async def _prerequisites_met(req: dict, orientation: str) -> tuple[bool, str | None]:
    ...
    from engine.utils.media_recovery import check_and_recover_media, get_local_path
...
async def _process_one(req: dict, deferred: dict = None, retry_after: dict = None):
    ...
    try:
        ...
        if _is_error(result):
            ...
            from engine.utils.slugify import slugify
            project_slug = slugify(project["name"]) if project else "project"
            from engine.utils.media_recovery import download_media_to_local
...
async def _dispatch(req: dict, orientation: str) -> dict:
    ...
    from engine.sdk.services.operations import get_operations
    ops = get_operations()
