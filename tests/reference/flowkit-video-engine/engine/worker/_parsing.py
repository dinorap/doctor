...
def _extract_media_id(result: dict, req_type: str) -> str:
    ...
    - Old: data.operations[].operation.metadata.video.*
    - New v3.1+: data.media[].video.operation.name (operation name is mediaId)
    """
    ...
    if req_type in ("GENERATE_VIDEO", "REGENERATE_VIDEO", "GENERATE_VIDEO_REFS", "UPSCALE_VIDEO"):
        # Try new v3.1+ format: media[].video.operation.name IS the mediaId
        media = data.get("media", [])
        if media:
            item = media[0]
            ...
            if op_name and _is_uuid(op_name):
                return op_name
            # Try generatedVideo.mediaId
            generated_video = video.get("generatedVideo", {})
            if isinstance(generated_video, dict):
                val = generated_video.get("mediaId", "")
                if val and _is_uuid(val):
                    return val
                fife = generated_video.get("fifeUrl", "")
                if fife:
                    uuid_val = _extract_uuid_from_url(fife)
                    if uuid_val:
                        return uuid_val
            # Fallback: use name/id from media item
            item_name = item.get("name", "") or item.get("id", "")
            if item_name and _is_uuid(item_name):
                return item_name
...
def _extract_output_url(result: dict, req_type: str) -> str:
    ...
    if req_type in ("GENERATE_VIDEO", "REGENERATE_VIDEO", "GENERATE_VIDEO_REFS", "UPSCALE_VIDEO"):
        # Try new v3.1+ format: media[].video.generatedVideo.fifeUrl
        media = data.get("media", [])
        if media:
            item = media[0]
            video = item.get("video", {})
            generated_video = video.get("generatedVideo", {}) if isinstance(video, dict) else {}
            if isinstance(generated_video, dict):
                url = generated_video.get("fifeUrl", "")
                if url:
                    return url
            # Fallback: video.fifeUrl
            if isinstance(video, dict):
                url = video.get("fifeUrl", "")
                if url:
                    return url
