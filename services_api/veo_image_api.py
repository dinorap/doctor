...
def build_generate_image_payload(
    ...
    *,
    ...
    # VEO API yêu cầu prompt tối thiểu ~15 ký tự. Nếu prompt quá ngắn, thêm mô tả để tránh lỗi 400.
    MIN_PROMPT_LENGTH = 15
    if len(prompt.strip()) < MIN_PROMPT_LENGTH:
        original_prompt = prompt.strip()
        prompt = f"{original_prompt}, high quality, detailed"
        print(f"[VEO Image API] ⚠️ Prompt quá ngắn ({len(original_prompt)} ký tự), đã mở rộng thành: {prompt}")
